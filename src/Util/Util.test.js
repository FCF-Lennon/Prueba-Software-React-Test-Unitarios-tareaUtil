...
describe('Pruebas Unitarias para la funcion Suma.', () => {
    ...
    it ('Verifica el tipo de dato si es distinto a un number lo invalida.', () => {
        let result1 = suma(8, 'test')
        if (typeof(result1) === 'string') {
            result1 = 'Dato Invalido.'
        }
        expect(result1).toBe('Dato Invalido.')
    });

    it ('Verifica el tipo de dato si es igual a number lo valida.', () => {
        let result2 = suma(8, 8)
        if (typeof(result2) === 'number') {
            result2 = 'Dato Valido.'
        }
        expect(result2).toBe('Dato Valido.')
    });

    it ('Verifica si los parametros de la funcion tienen un valor asignado.', () => {
        expect(suma("", 1)).toBe('error - parametro en blanco.')
    });
